...
<?php if ( function_exists( 'wp_body_open' ) ) { wp_body_open(); } ?>
    ...
    <!-- Barra de información editable -->
    <div class="w-full bg-tertiary text-white text-sm py-2 px-4 md:px-0">
...
<!-- Menú Mobile (fullscreen desde arriba) -->
<div id="mobile-menu" class="fixed inset-0 bg-white z-50 transform -translate-y-full transition-transform duration-300 ease-in-out sm:hidden">
    <!-- Botón de cerrar arriba a la derecha -->
    <button id="menu-close" class="absolute top-4 right-4 text-gray-800 text-3xl focus:outline-none">
        <i class="fa-solid fa-xmark"></i>
    </button>

    <div class="p-6 pt-16 space-y-6"> <!-- pt-16 para dejar espacio al botón de cerrar -->
        <?php
        wp_nav_menu([
            'theme_location' => 'primary',
            'menu_class'     => 'flex flex-col space-y-4 text-lg font-medium uppercase text-center',
            'container'      => false,
        ]);
        ?>
        ...
        <!-- Botón Tienda (mobile) -->
        <a href="<?php echo get_permalink( wc_get_page_id('shop') ); ?>"
           class="bg-primary text-white px-4 py-2 font-medium hover:bg-secondary transition uppercase block text-center rounded-xs">
           Tienda
        </a>
    </div>
</div>