...
function Save({ attributes }) {
  ...
  return createElement(
    ...
    cards.map((card, idx) => {
      ...
      const hasHoverImage = !!(card.hoverBgImage && hasLink);
      const isWhiteHoverCard = hasHoverImage && (card.bgColor || "white") === "white";
      const textColorClass = isWhiteHoverCard ? "" : card.bgColor === "white" ? "text-black" : "text-white";
      ...
      const cardProps = {
        key: idx,
        className: `bg-card ${hasHoverImage ? "" : bgColorClass} ${textColorClass} ${colSpanClass} h-[350px] md:h-[300px] lg:h-[400px] p-6 md:p-8 ${hasLink ? "card-item" : "text-center md:text-start"}${hasHoverImage ? " card-item--has-hover" : ""}${isWhiteHoverCard ? " card-item--hover-white" : ""}`,
        style: {
          position: "relative",
          overflow: "hidden",
          display: "flex",
      ...
      return createElement(
        Tag,
        cardProps,
        // Imagen de fondo + overlay de color (solo si tiene imagen hover y link)
        hasHoverImage && createElement("div", {
        ...
        hasHoverImage && createElement("div", {
          className: `card-bg-overlay card-bg-overlay--${card.bgColor || "primary"}`
        }),
        // Background hover legacy (sin overlay — fallback)
        !hasHoverImage && card.hoverBgImage && hasLink && createElement("div", {
          className: "card-bg-hover",
          style: {
            backgroundImage: `url(${card.hoverBgImage})`,
